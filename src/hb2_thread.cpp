...
sensor_msgs::PointCloud2 pc2_area0;
...
void callback(const PointCloud2::ConstPtr &point_cloud, const autobox_out::ConstPtr &can_data)
{
  ...
  //Use localization method depending on GPS signal quality
    ...
    if (measure_state == MeasureState::Laser)
    {
      ...
      // std::thread t1([&pc](){
      //   std::cout << "points num:"<< pc.points.size() <<"\n";
      // });

      std::cout << "scan num:"<< pc.points.size() <<"\t";

      MyPointCloud2D scans_Area0;

    
      std::thread t0([&scans,&scans_Area0]() mutable{
        scans_Area0 = filter.getScanPointsWithinThreshold_param(scans,5,-5,5,-5);
        
        Matrix2d rotM = filter.allignScanPoints(scans_Area0, state);

        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_source(new pcl::PointCloud<pcl::PointXYZ>);

        saveMyPointCloudtoPCLXYZ(scans_Area0,cloud_source);
        std::cout << "Before Filter Size:" << cloud_source->size()<< std::endl;

        pcl::VoxelGrid<pcl::PointXYZ> sor;
        sor.setInputCloud (cloud_source);
        sor.setLeafSize (0.01f, 0.01f, 10.0f);
        sor.filter (*cloud_source);

        std::cout << "After Filter Size:" << cloud_source->size()<< std::endl;


        pc2_area0 = rviz.createPointCloud(scans_Area0, "ibeo_lux", 1.1);
      });

      
      // std::thread t1([&scans](){
      //   MyPointCloud2D scans_Area1 = filter.getScanPointsWithinThreshold_param(scans,25,5,5,-5);
      //   std::cout << "scans_Area1 num:"<< scans_Area1.pts.size() <<"\n";
      // });


      // std::thread t2([&scans](){
      //   MyPointCloud2D scans_Area2 = filter.getScanPointsWithinThreshold_param(scans,25,5,15,5);
      //   std::cout << "scans_Area2 num:"<< scans_Area2.pts.size() <<"\n";
      // });


      // std::thread t3([&scans](){
      //   MyPointCloud2D scans_Area3 = filter.getScanPointsWithinThreshold_param(scans,5,-5,15,5);
      //   std::cout << "scans_Area3 num:"<< scans_Area3.pts.size() <<"\n";
      // });
      ...
      // std::thread t4([&scans](){
      //   MyPointCloud2D scans_Area4 = filter.getScanPointsWithinThreshold_param(scans,-5,-15,10,5);
      //   std::cout << "scans_Area4 num:"<< scans_Area4.pts.size() <<"\n";
      // });

      // std::thread t5([&scans](){
      //   MyPointCloud2D scans_Area5 = filter.getScanPointsWithinThreshold_param(scans,-5,-25,5,-5);
      //   std::cout << "scans_Area5 num:"<< scans_Area5.pts.size() <<"\n";
      // });


      // std::thread t6([&scans](){
      //   MyPointCloud2D scans_Area6 = filter.getScanPointsWithinThreshold_param(scans,-5,-25,-5,-15);
      //   std::cout << "scans_Area6 num:"<< scans_Area6.pts.size() <<"\n";
      // });
      

      // std::thread t7([&scans](){
      //   MyPointCloud2D scans_Area7 = filter.getScanPointsWithinThreshold_param(scans,5,-5,-5,-15);
      //   std::cout << "scans_Area7 num:"<< scans_Area7.pts.size() <<"\n";
      // });


      // std::thread t8([&scans](){
      //   MyPointCloud2D scans_Area8 = filter.getScanPointsWithinThreshold_param(scans,25,5,-5,-15);
      //   std::cout << "scans_Area8 num:"<< scans_Area8.pts.size() <<"\n";
      // });
      ...
      // t8.join();
      t0.join();
      std::cout << "scan_Area0  num:"<< scans_Area0.pts.size() <<"\n";
    }
...
int main(int argc, char **argv)
{
  ...
  auto area0_pub = nh.advertise<sensor_msgs::PointCloud2>("Area0", 30);
  ...
  while (ros::ok())
  {
    ...
    //Point Cloud Map
    pc2_area0.header.stamp = ros::Time::now();
    area0_pub.publish(pc2_area0);
